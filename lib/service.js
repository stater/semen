...
class Service {
    ...
    constructor(name, options = {}) {
        ...
        if (Object.keys(options).length > 0) {
            ...
            logger.debug(`Wrapping service ${color.magenta(name)}...`).incIndent();
        }
    ...
    init() {
        ...
        if ('string' === typeof this.configs) {
            this.configs = $main.configsStore.get(this.configs) || {};
        } else if (Array.isArray(this.configs)) {
            this.configs.forEach((cfg, i) => {
                if ('string' === typeof cfg) {
                    this.configs[ i ] = $main.configsStore.get(cfg) || {};
                }
            });
        } else {
            if ('undefined' === typeof this.configs) {
                this.configs = $main.configsStore.get(`${this.name}-config@~${this.version}`) || {};
            }
        }
            ...
            this.services.forEach((service, i) => {
                ...
                if ('string' === typeof service) {
                    ...
                    if (service) {
                        ...
                        throw new Error(`Invalid Service: Service ${color.magenta(name)} cannot be found!`);
                    }
                }
    ...
    wrap(context) {
        ...
        let clone = Object.assign(new Service(this.name), this);
     ...
     * @param {object} [parent] - A constructor to apply to the fn.
     * @returns {*}
     */
    exec(fn, parent) {
        ...
        // Throw new error if the service doesn't have a context.
            ...
            throw new Error('Invalid Context: Service call require context in the service it self.');
        }
    }
    ...
    async() {
        if (this.status !== 'ready') {
            throw new Error(`Illegal Start: Service ${color.magenta(this.name)} must be initialized before it can started!`);
        }
                ...
                if ('service' === typeOf(service)) {
                    ...
                    // Get the handler function from the service if the current service is an object.
                        ...
                        if ('string' === typeof service.handler) {
                            ...
                            let fn = service[ service.handler ];

                            if ('function' === typeof fn) {
                                // If the handler found, call it.
                                this.exec(fn, service);
                            } else {
                                throw new Error(`Invalid Hanlder: The required handler ${color.magenta(service.handler)} from ${color.yellow(service.name)} is not a function or undefined.`);
                            }
                        } else {
                            throw new Error(`Invalid Service: The service "${logger.color.yellow(service.name)}" doesn't have function to start.`);
                        }
                    }
                }
            });
        ...
        // Start single service if the services is not an array.
            ...
            // Get the handler function if this.services is an object.
                ...
                if ('string' === typeof this.handler) {
                    // Get the handler function from the service.
                    let fn = this.services[ this.handler ];

                    if ('function' === typeof fn) {
                        // If the handler found, call it.
                        this.exec(fn, this.services);
                    } else {
                        throw new Error(`Invalid Hanlder: The required handler ${color.magenta(this.handler)} from ${color.yellow(this.name)} is not a function.`);
                    }
                } else {
                    throw new Error(`Invalid Service: The service "${logger.color.yellow(this.name)}" doesn't have function to start.`);
                }
            }
        }
        ...
        return new Promise((resolve, reject) => {
                ...
                foreach(this.services).run((i, service, next) => {
                    ...
                    if ('service' === typeOf(service)) {
                        ...
                        if ('string' === typeof service.handler) {
                            ...
                            if ('function' === typeof fn) {
                                result = this.exec(fn, service);
                            } else {
                                throw new Error(`Invalid Hanlder: The required handler ${color.magenta(service.handler)} from ${color.yellow(service.name)} is not a function.`);
                            }
                        } else {
                            throw new Error(`Invalid Service: The service "${logger.color.yellow(service.name)}" doesn't have function to start.`);
                        }
            ...
            // Use single service call if this.services is not an array.
                ...
                if ('function' === typeof this.services) {
                    ...
                    result = this.exec(this.services);
                } else if ('[object Object]' === toString.call(this.services)) {
                    ...
                    if ('string' === typeof this.handler) {
                        ...
                        if ('function' === typeof fn) {
                            result = this.exec(fn, this.services);
                        } else {
                            throw new Error(`Invalid Hanlder: The required handler ${color.magenta(this.handler)} from ${color.yellow(this.name)} is not a function.`);
                        }
                    } else {
