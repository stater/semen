...
class Stater {
    constructor() {
        ...
        // Makes the helper invisible.
        Object.defineProperty(this, 'helper', { enumerable: false });
    }
    ...
    initialize() {
        ...
        Object.keys(this.configsStore.configs).forEach(name => {
            let cfgGroup = this.configsStore.configs[ name ];

            Object.keys(cfgGroup).forEach(version => {
                let cfg = cfgGroup[ version ];

                if ('function' === typeof cfg.data && cfg.data.initialize) {
                    logger.debug(`Initializing config of ${color.magenta(`${name}@${version}`)}...`);

                    let params = this.resolve(cfg.data);

                    cfg.data = cfg.data.apply(cfg.data, params);
                }
            });
        });
